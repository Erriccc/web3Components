...
const LiquidityPositionPage = () => {
    ...
    return (<>
      <NextSeo title="Liquidity Position" description="Map3 - React Next Web3 NFT Crypto Dashboard Template"/>
      <DashboardLayout>
        <div className="mx-auto w-full max-w-lg text-sm">
          ...
          <div className="mt-6 rounded-lg bg-white p-4 shadow-card dark:bg-light-dark sm:p-6">
            <div className="rounded-lg border border-solid border-gray-200 bg-body dark:border-gray-700 dark:bg-dark">
              <div className="flex h-16 w-full cursor-pointer items-center justify-between p-3" onClick={() => setIsExpand(!isExpand)}>
                <CurrencySwapIcons from="BTC" to="ETH"/>

                <span className="flex items-center text-sm text-gray-500 dark:text-gray-300">
                  Manage
                  <ChevronDown className={`transition-all ltr:ml-1.5 rtl:mr-1.5 ${isExpand ? 'rotate-180' : 'rotate-0'}`}/>
                </span>
              </div>
              <AnimatePresence initial={false}>
                {isExpand && (<motion.div key="liquidity-content" initial="collapsed" animate="open" exit="collapsed" variants={{
                ...
                collapsed: { opacity: 0, height: 0 },
                    ...
                    <div className="border-t border-dashed border-gray-200 p-4 dark:border-gray-700 sm:p-6">
                      <div className="flex flex-col gap-3 xs:gap-[18px]">
                        <TransactionInfo label="YOUR TOTAL POOL TOKEN:" value="22.51"/>
                        <TransactionInfo label="POOLED BTC:" value="0.01940272 BTC"/>
                        <TransactionInfo label="POOLED ETH:" value="0.14689574 ETH"/>
                        <TransactionInfo label="YOUR POOL SHARE:" value="0.06%"/>
                      </div>
                      <Button size="large" shape="rounded" fullWidth={true} color="gray" className="mt-6 uppercase dark:bg-gray-800">
                        Remove
                      </Button>
                    </div>
                  </motion.div>)}
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              ...
              <Button size="large" shape="rounded" fullWidth={true} color="gray" className="uppercase dark:bg-gray-800">
                Import
              </Button>
            </div>
          </div>
